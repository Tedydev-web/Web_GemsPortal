...
function ChangePageTraining(page, e, elm) {
    e.preventDefault();
...
function LoadPagination(totalRecords, pageSize = 12, currentPage) {
    ...
    if (totalPage >= 1) {
        ...
        if (currentPage > 1) {
            html += `
                <li class="page-item">
                    <a class="page-link" href="${link}record=${pageSize}&page=${currentPage - 1}" title="Trang trước" onclick="ChangePageTraining(${currentPage - 1},event,this)"
                    aria-label="Previous">
        ...
        for (var i = startPage; i <= endPage; i++) {
            if (currentPage == i) {
                ...
                html += `<li class="page-item">
                            <a class="page-link" href="${link}record=${pageSize}&page=${i}" onclick="ChangePageTraining(${i},event,this)" title="Trang ${i}">${i}</a>
                        </li>`;
            }
        }
        if (currentPage < totalPage) {
            html += `<li class="page-item">
                        <a class="page-link"  href="${link}record=${pageSize}&page=${currentPage + 1}" title="Trang kế tiếp"  onclick="ChangePageTraining(${currentPage + 1},event,this)"
                            aria-label="Next">
                            <span aria-hidden="true">
