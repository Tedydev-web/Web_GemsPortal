            ...
            success: function (response) {
                ...
                if (listData != null && listData.length > 0) {
                    $.each(listData, function (key, item) {
                        tmpHtml += `
                            <li class="btn_supplier" onclick="HandleClickSupplier(this)" value="${item.id}" data-nameShop="${item.name}" data-logo="${!IsNullOrEmty(item.imageLogo) ? item.imageLogo : ''}" data-lat="${item.latitude}" data-long="${item.longitude}" data-name="${item.name}">
                                <div class="media">
...
function ShowModalRegister(elm, type) {
    ...
    switch (type) {
        case 0:
            $.get("/Member/P_RegisterDistributor").done(function (response) {
