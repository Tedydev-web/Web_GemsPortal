        ...
        source: {
            product: {
                ...
                template: function (query, item) {
                    return `<div class="man-section"><div class="image-section"><img style="border-radius:4px;" src="{{imageObj.smallUrl}}" onerror="this.onerror=null;this.src='/assets/images/error/product_1x1_medium.png';"></div><div class="description-section"><h4 class="sp-line-2">{{name}}</h4><span class="text-danger" title="${IsNullOrEmty(item.name) ? "" : item.name}">${item.supplierObj?.isMember > 0 ? CERTIFICATE_MEMBER : ''}${RawSearchProductPrice(item.productPriceObjs)}${RawSearchProductRatioDiscount(item.productPriceObjs)}</div></div>`;
                }
            }
        },
