...
{
    public class QualityController : BaseController<QualityController>
    {
        private readonly IS_News _s_News;
        private readonly IOptions<Config_MetaSEO> _metaSEO;

        public QualityController(IS_News news, IOptions<Config_MetaSEO> metaSEO)
        {
            _s_News = news;
            _metaSEO = metaSEO;
        }
        ...
        public async Task<IActionResult> Index(int? c, int record = 12, int page = 1)
        {
            ...
            await Task.WhenAll(GetListNews(), GetListIntroduce());

            SetViewDataSEOExtensionMethod.SetViewDataSEODefaultAll(this, _metaSEO.Value.Quality);
            return View();
        }
        ...
        {
            ResponseData<List<M_News>> res = await _s_News.getListHubNewsByCategoryIdTypeId(_supplierId, c, EN_NewsCategoryTypeId.quantity, record, page);
